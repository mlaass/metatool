...
std::string fill_template(std::string templ,
                          const std::vector<str_pair> &args) {
...
void parseXMLMemberStructTemplate(
    tinyxml2::XMLElement *root, const std::string &type,
    std::map<std::string, metatool::MemberStructTemplate> &target) {

  for (auto *e = root->FirstChildElement(type.c_str()); e != NULL;
...
void parseXMLTemplates(std::istream &stream, auto &structTemplates,
                       auto &fileTemplates,
  ...
  try {
    ...
    parseXMLMemberStructTemplate(root, "member",
                                 StructDeclASTVisitor::memberTemplates);
    parseXMLMemberStructTemplate(root, "method",
                                 StructDeclASTVisitor::methodTemplates);
    parseXMLMemberStructTemplate(root, "function",
                                 StructDeclASTVisitor::functionTemplates);
    parseXMLMemberStructTemplate(root, "argument",
...
void parseT8MemberStructTemplate(
    Definition &d,
    std::map<std::string, metatool::MemberStructTemplate> &target) {
...
void parseT8Templates(std::istream &stream, auto &structTemplates,
                      auto &fileTemplates,
                      const std::string &outputFilename = "") {
...
int generate(
  ...
  const llvm::ArrayRef<std::__cxx11::string> &files,
  std::istream &stream,
  bool t8 = true,
  const std::string &outputFilename = "")
  {
